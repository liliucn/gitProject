...
# region 读取本地excel表格内的数据集（抽取每类60%样本组成训练集，剩余样本组成测试集）
...
# region 读取本地excel表格内的数据集（每类随机生成K个训练集和测试集的组合）
...
# region 线性鉴别分析（Linear Discriminant Analysis）
def LDA():
...
# region支持向量机（Support Vector Machine）
def SVM():
    clf = svm.SVC()
    return clf
...
# region逻辑回归（Logistic Regression）
def LR():
...
# region GBDT GradientBoostingClassifier
...
# regionreport函数，将调参的详细结果存储到本地F盘（路径可自行修改，其中n_top是指定输出前多少个最优参数组合以及该组合的模型得分）
...
# region“主”函数1（KFold方法生成K个训练集和测试集，即数据集采用getData_3()函数获取，计算这K个组合的平均识别率）
...
# region“主”函数2（每类前x%作为训练集，剩余作为测试集，即数据集用getData_2()方法获取，计算识别率）
